...
| D-020 | 2026-07-12 | Protect anonymous contributions with exact-origin checks, server-side Turnstile, canonical network identity, global plus endpoint/contributor database rate buckets, endpoint-and-network-actor-scoped stable-receipt idempotency kept separate from semantic contributor deduplication, configured versioned retention, and follow-up rows created only by typed qualifying events | Contains abuse across serverless instances without storing raw network addresses; collapses retries/duplicates without letting mutable contact data redefine idempotency; preserves endpoint isolation and the documented NAT-undercounting tradeoff; production fails closed without retention and least-privilege service-role configuration; consent alone never creates mail work or public content | Counsel changes consent/retention wording, a provider changes validation semantics, measured abuse changes rate policy, or accounts replace anonymous identity |
| D-021 | 2026-07-13 | Use two private Supabase Storage buckets, a five-minute single-object app capability, exact-intake binding, quarantine decoding, metadata-free WebP derivatives and manual rectangle redaction; keep all WP-09 media private | Avoids exposing service credentials or accepting the provider's longer native upload capability; prevents semantic aliases sharing media; preserves separate public-display consent | Counsel selects final wording/retention, provider controls change, or a reviewed later package authorizes public derivatives |
| D-022 | 2026-07-13 | Keep all real source-platform automation disabled in WP-10; ship a demo/test-only Thingiverse-shaped fixture adapter plus manual/creator intake and allowlisted link health | Current official-source review did not establish project-specific credentials and permission for the intended commercial automation; scraping or inferred permission would violate the source-policy boundary | A reviewer records current official API or written permission, exact allowed fields, commercial compatibility, credentials and an enabled policy snapshot |
| D-023 | 2026-07-13 | Make public indexing depend on one shared fail-closed policy: exact `DEMO_MODE=false`, a strict canonical origin, production Node/provider state, exact `REPAIRPRINT_DEPLOYMENT_ENV=production`, and a configured HTTPS notice channel | Preview and staging must remain blocked independently of URL shape, and page metadata, robots, proxy headers and the sitemap must not drift into separate eligibility rules | A reviewed release design replaces any launch marker or origin/contact prerequisite with equally testable fail-closed controls |
| D-024 | 2026-07-13 | Keep WP-11 analytics first-party, aggregate-only, vendor-neutral and disabled until an exact mode plus dedicated execute-only database credential are reviewed | The event contract needs useful zero-result and ambiguity counts without retaining raw searches, contribution payloads, network/browser identifiers or event rows; no provider or retention decision exists | Product, privacy and operations owners approve a provider/retention horizon or measured needs require a versioned privacy-preserving alternative |
| D-025 | 2026-07-13 | Define the WP-11 analytics function boundary as one direct recorder grant and one directly callable non-extension application routine, while permitting only the exact fingerprinted `pg_trgm` 1.6 `PUBLIC EXECUTE` baseline | PostgreSQL grants extension functions to `PUBLIC`; removing that baseline would mutate unrelated provider-owned ACLs, while a canonical provenance/owner/security/ACL manifest can distinguish the approved extension from application authority | `pg_trgm` version, location, owner, routine set, security properties or ACLs change; another extension needs an exception; or PostgreSQL adds a narrower deny mechanism |
