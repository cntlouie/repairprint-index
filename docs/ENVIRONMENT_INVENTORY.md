...
| `DATABASE_DIRECT_URL` | Local Docker direct credential from `.env.example` | Not required | Not configured | GitHub secret `STAGING_DATABASE_DIRECT_URL` rotated with the provider password on 2026-07-12 | Deferred | Server-only; never expose to application clients |
