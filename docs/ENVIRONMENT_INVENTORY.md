...
| --- | --- | --- | --- | --- | --- |
| Local | Application ready; database runtime availability is checked by the guarded gate | Node.js 22.22.0 observed; npm 10.9.4; Docker fixture defines separate `repairprint` and guarded `repairprint_test` databases with the 28-table WP-08 migration set | `true` by template and fail-closed when absent | `http://localhost:3000` | Disallow all, empty sitemap, page-level `noindex` |
...
| `SUBMISSION_HMAC_SECRET` | Empty in non-persisting demo; 32+ byte local secret for production-mode tests | Ephemeral CI secret/fixture | Server-only preview secret if persistent intake is enabled | Must be provisioned before real staging intake | Required before production intake | Server-only; domain-separated HMAC for idempotency, dedupe, and pseudonymous rate/contributor keys |
| `SUBMISSION_DATABASE_URL` | Empty unless production-mode intake is being tested | Ephemeral guarded credential for the named role | Never needed for demo-only preview | Must be provisioned for `repairprint_submission_service` before persistent intake | Required before production intake | Server-only named-role credential; runtime rejects owner or any other PostgreSQL identity |
