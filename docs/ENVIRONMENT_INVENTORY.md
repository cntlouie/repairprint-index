...
| Staging/demo | Web and managed database provisioned; migration/restore evidence pending | Vercel Hobby project `repairprint-index`; Supabase Free project `repairprint-index-staging` (`inscdebgwdzubyzfifkd`) in West EU (Ireland); public Data API disabled | `true` | `https://repairprint-index.vercel.app` | Verified 200 responses, disallow-all robots, empty sitemap, and page-level `noindex` on 2026-07-11 |
...
| `RESTORE_DATABASE_URL` | Not configured | Temporary GitHub Actions secret during an isolated restore drill | Never configured | Temporary only; remove after the restore project is deleted | Never configured | Must never target staging or production |
