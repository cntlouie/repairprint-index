...
| Staging/demo | Web and managed database ready; WP-01 migration, seed, encrypted backup, and isolated restore drill passed | Vercel Hobby project `repairprint-index`; Supabase Free project `repairprint-index-staging` (`inscdebgwdzubyzfifkd`) in West EU (Ireland); public Data API disabled | `true` | `https://repairprint-index.vercel.app` | Verified 200 responses, disallow-all robots, empty sitemap, page-level `noindex`, 22-table migration, idempotent fictional seed, encrypted artifact, and zero-published-fitment restore audit on 2026-07-11 |
