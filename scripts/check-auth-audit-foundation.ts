...
async function main(): Promise<void> {
  ...
  try {
    ...
    const [submissionCleanupBoundary] = await sql<{
    ...
    `;
    if (!submissionCleanupBoundary
      || submissionCleanupBoundary.missingRoutines !== 0
      || submissionCleanupBoundary.unexpectedOverloads !== 0
      || submissionCleanupBoundary.hardenedDefinitionViolations !== 0
      || submissionCleanupBoundary.hardenedAclDelta !== 0
      || submissionCleanupBoundary.publicEffectiveExecute !== 0
      || submissionCleanupBoundary.deniedEffectiveExecute !== 0
      || submissionCleanupBoundary.allowedExecuteMissing !== 0
      || submissionCleanupBoundary.serviceRoutineAclDelta !== 0
      || submissionCleanupBoundary.seventhDefinitionViolations !== 0
      || submissionCleanupBoundary.seventhAclDelta !== 0
      || submissionCleanupBoundary.defaultFunctionAclDelta !== 0
      || submissionCleanupBoundary.defaultFunctionAclRows !== 1) {
      throw new Error(
        `Submission cleanup routine boundary is invalid: ${JSON.stringify(submissionCleanupBoundary)}.`,
      );
    }
